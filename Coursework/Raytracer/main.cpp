...
int main(int argc, char* argv[]) {
    ...
    std::vector<uint8_t> spideyHeadTex, spideyBodyTex, spideyEquipTex, spideyLegsTex;
    std::vector<uint8_t> asgardTex, pillarTex, marbleTex, palenquinTex;

    unsigned w1, h1, w2, h2, w3, h3, w4, h4, w5, h5, w6, h6, w7, h7, w8, h8;

    lodepng::decode(spideyBodyTex, w1, h1, "../models/T_1036800_Body_D.png");
    lodepng::decode(spideyEquipTex, w2, h2, "../models/T_1036800_Equip_01_D.png");
    lodepng::decode(spideyHeadTex, w3, h3, "../models/T_1036800_Head_D.png");

    lodepng::decode(asgardTex, w4, h4, "../models/T_AsgardC02BuildingIND001Aa_D.png");
    lodepng::decode(pillarTex, w5, h5, "../models/T_AsgardE01Pillar006A_D.png");
    lodepng::decode(marbleTex, w6, h6, "../models/Marble.png");

    lodepng::decode(spideyLegsTex, w7, h7, "../models/T_1036800_Equip_02_D.png");
    lodepng::decode(palenquinTex, w8, h8, "../models/DarkGold.png");

    TexturedLambertianShader spideyHeadShader(&spideyHeadTex, w3, h3);
    TexturedLambertianShader spideyBodyShader(&spideyBodyTex, w1, h1);
    TexturedLambertianShader spideyEquipShader(&spideyEquipTex, w2, h2);
    TexturedLambertianShader spideyLegsShader(&spideyLegsTex, w7, h7);

    TexturedLambertianShader asgardShader(&asgardTex, w4, h4);
    TexturedLambertianShader pillarShader(&pillarTex, w5, h5);
    TexturedLambertianShader floorShader(&marbleTex, w6, h6);
    TexturedLambertianShader palenquinShader(&palenquinTex, w8, h8);
    ...
    scene.renderables.push_back(std::make_shared<BVHNode>(CharacterMesh1, &spideyHeadShader, 4, baseTransform));
    scene.renderables.push_back(std::make_shared<BVHNode>(CharacterMesh2, &spideyBodyShader, 4, baseTransform));
    scene.renderables.push_back(std::make_shared<BVHNode>(CharacterMesh3, &spideyEquipShader, 4, baseTransform));
    scene.renderables.push_back(std::make_shared<BVHNode>(CharacterMesh4, &spideyLegsShader, 4, baseTransform));

    scene.renderables.push_back(std::make_shared<BVHNode>(CharacterMesh5, &asgardShader, 4,
        makeTranslation(-0.4f, -2.0f, 12.f) * makeRotateY(M_PI - 0.2f)));

    scene.renderables.push_back(std::make_shared<BVHNode>(CharacterMesh6, &palenquinShader, 4,
        makeTranslation(-1.0f, -2.4f, 12.f) * makeRotateY(M_PI - 0.2f)));

    scene.renderables.push_back(std::make_shared<BVHNode>(CharacterMesh7, &pillarShader, 4,
        makeTranslation(-0.2f, -2.4f, 15.f) * makeRotateY(M_PI - 0.2f)));

    scene.renderables.push_back(std::make_shared<BVHNode>(CharacterMesh8, &pillarShader, 4,
        makeTranslation(-7.0f, -3.0f, 10.f) * makeRotateY(M_PI - 0.2f)));

    scene.renderables.push_back(std::make_shared<BVHNode>(CharacterMesh9, &floorShader, 4,
